...
module Exchanges
  class Bkex < BaseExchange
    ...
    def symbols
      ...
      data.reject do |symbol|
        symbol.index('3L') || symbol.index('3S') || symbol.index('5L') || symbol.index('5S') || symbol.index('BULL') || symbol.index('BEAR')
      end
    end
    ...
    def ignore_symbol?(coin_name)
      %w[5CDP AIOT AZP BCE BHG BITC BKF CKZ70 CMC CTFTT DATA DCNT DDD DSC DT DTO EDG EDP EIDOS FM GOT
